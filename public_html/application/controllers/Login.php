				...
				if($row->estado == "1") {
					if($row->clave == $this->input->post('contrasena')) {
						...
						if($cambio_clave == '1'){		
							...
							$datos_session= array('C_id_usuario'=>$id_usuario, 'C_id_empleado'=>$id_empleado, 'C_nombre_usuario'=>$nombre_usuario, 'C_nom_usuario'=>$nom_usuario, 'C_perfil'=>$perfil, 'C_tipo'=>$tipo, 'C_foto'=>$foto, 'C_origen'=>$this->session->userdata('C_basedatos') );
