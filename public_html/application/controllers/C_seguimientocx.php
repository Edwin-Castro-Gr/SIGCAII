...
class C_seguimientocx extends CI_Controller {
	...
	public function modificar($id) {
		...
		redirect(base_url());
			...
			$data_usua['c_ayudante'] = '';
			$data_usua['c_mat'] = '';
			$data_usua['c_estado'] = '';
			$data_usua['c_tiempo'] = '';
			$data_usua['c_ubicacion_paciente'] = '';
			$data_usua['c_usuario'] = '';

			$campos ='id_cirugia, id_paciente, nombres, edad, genero, Telefono_paciente, direccion, id_eps, id_entidad, fecha_Cx, hora_Cx, sala, procedimiento, id_cirujano, id_anest, ayudante, mat, tiempo, ubicacion_paciente, estado, usuario';
		...
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" )
			...
			if(!$this->input->is_ajax_request()) {
				...
				$registro=array(
					...
					'ayudante'=>$this->input->post('ayudantes_programacion'),					
	...
	public function guardar_cancelacion() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" )
			redirect(base_url());
		else {
			if(!$this->input->is_ajax_request()) {
				...
				$fecha = date('Y-m-d H:i:s');
				$registro = array(
					'id_curso'=>$this->input->post('id_cirugiap'),
				...
				//echo var_dump($registro);
				$query = $this->general_model->insert('p_curso_cx_cancelacion', $registro);
				// echo '1';
				...
				$query2 = $this->general_model->update('p_curso_cx', 'id_cirugia', $this->input->post('id_cirugiap'), $registro1);
		...
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" )
			...
			if(!$this->input->is_ajax_request()) {
				...
				if($estado=="2"){
					...
					if($correo2!=""|| $correo2.length!= 0){ //<<<-------- ENVIO DE EMAIL CASA COMERCIAL 2------->>>//
