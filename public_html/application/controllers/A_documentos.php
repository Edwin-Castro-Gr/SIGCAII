	...
	public function consecutivo1() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				if($macro=="" && $proce =="" &&$docrelacionado=="" && $tipod!=""){ //CONSECUTIVO PARA DOCUMENTO EXTERNO
				...
				if($macro!="" && $proce =="" && $docrelacionado=="" && $tipod!=""){
				...
				if($macro !="" && $proce =="" && $subproce=="" && $docrelacionado !="" && $tipod !=""){
				...
				if($proce !="" && $subproce == "" && $docrelacionado =="" && $tipod !=""){
					//CONSECUTIVO PARA TRANSVERSALES DE LOS PRCESOS SIN DOCUMENTOS RELACIONADOS
				...
				if($proce !="" && $docrelacionado !="" && $tipod !=""){
					//CONSECUTIVO PARA TRANSVERSALES DE LOS PRCESOS CON DOCUMENTOS RELACIONADOS
					$valdocrela = explode(",",$docrelacionado);
					$docrela = end($valdocrela);
				...
				if($proce !="" && $subproce!="" && $docrelacionado =="" && $tipod !="" && $clase== "5"){
				...
				}else if($proce !="" && $subproce!="" && $docrelacionado =="" && $tipod !=""){
					//CONSECUTIVO DOCUMENTOS DE LOS SUBPRCESOS SIN DOCUMENTOS RELACIONADO					
	...
	public function guardar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$doc_relacionados = $this->input->post('doc_relacionados');
				$val_docrela = implode(',', (array) $doc_relacionados);
				...
				if($idsolicitud=="00"){
					...
					if ($tipo_Solicitud == '1'){
						...
						$registro=array(
							...
							'docrelacionado'=>$this->input->post('doc_relacionados'),
							'codigo'=>$this->input->post('codigo'),
							'evaluacion'=>$this->input->post('evalua'),
							'des_empleados'=>$val_empleado,
							'des_cargos'=>$val_cargo,
						...
						if(is_array($id_empleado)){//Cuando son varios funcionarios
							foreach ($id_empleado as $value) {
								...
								if($query >= 1) {	
									...
									$de="Calidad CECIMIN <castonino17@gmail.com>";
