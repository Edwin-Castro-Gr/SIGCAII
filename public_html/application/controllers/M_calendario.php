...
class M_calendario extends CI_Controller
{
	...
	public function modificar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$campos= 'mp.id_programacionm AS "Id", ms.id_solicitud AS "Id Solicitud", ms.id_manterimientor AS "Id_Mantenimiento", mr.nombre AS "Mantenimiento", ms.id_servicio AS "Id_Servicio", se.nombre AS "Servicio", ms.ubicacion AS "Ubicación", ms.otros_mantenimientos AS "Otro", ms.observaciones AS "Observaciones", ms.id_solicitante AS "Id_Solicitante", IFNULL(CONCAT(em.nombres, " ", em.apellidos),"") AS "Solicitante", em.email AS "Correo_Solicitante",mp.tipo_mantenimiento AS "Tipo Mantenimiento", ms.estado AS "Estado"';
			$query = $this->general_model->consulta_personalizada($campos,'mantenimientos_solicitudes ms INNER JOIN mantenimientos_programacion mp ON ms.id_solicitud=mp.id_solicitudm INNER JOIN mantenimientos_requeridos mr ON ms.id_manterimientor=mr.id_mantenimiento_r INNER JOIN servicios se ON ms.id_servicio=se.id_servicio INNER JOIN empleados em ON ms.id_solicitante=em.id_empleado','mp.id_programacionm ='.$id.'', '', 0, 0);
			$row = $query->row_array();

			$arr['mantenimiento'] = array('Id' => $row['Id'], 'id_solicitud'=>$row['Id Solicitud'], 'Id_Mantenimiento'=>$row['Id_Mantenimiento'],'Id_Servicio'=>$row['Id_Servicio'],'Ubicación'=>$row['Ubicación'],'Otro'=>$row['Otro'],'Observaciones'=>$row['Observaciones'],'Id_Solicitante'=>$row['Id_Solicitante'], 'Correo_Solicitante'=>$row['Correo_Solicitante'],'Tipo_Mantenimiento'=>$row['Tipo Mantenimiento'], 'Estado'=>$row['Estado']);
