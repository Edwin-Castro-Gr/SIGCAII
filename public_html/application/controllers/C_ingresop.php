	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_ingresosp_tabla('WEB');
			}
		}
	}
		...
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				if($query >= 1){
					...
					$cuerpo .= "<br>\r\n";
					$cuerpo .= "<br>\r\n";
					$cuerpo .= "<div><font size='3'>Acontinuación nos permitimos relacionar los documentos necesarios para continuar con el proceso de Ingreso a Cecimin:</font></div>\r\n";
					$cuerpo .= "<br>\r\n";
					...
					{
						$i++;
						$cuerpo .= "<div><font size='3'><b>".$i++."). ".$row['Nombre']."</b></font></div>\r\n";
						
					}
				    $enviarLink = base_url().'c_enviaringresop/index?idcargo='.$cargo.'&idingreso='.$idingreso.'&usuario='.$usuario.'';

				    $cuerpo .= "<div><font size='3'><a href='".$enviarLink."'>Haga click aquí</a> para abrir el formulario y cargar los documentos en formato PDF</font></div>\r\n";
				    $cuerpo .= "<br>\r\n";		
				    $cuerpo .= "<br>\r\n";
				    $cuerpo .= "<br>\r\n";	
				    $cuerpo .= "<div><font size='3'>Agradeciendo la atención prestada,</font></div>\r\n";
				    $cuerpo .= "<br>\r\n";		
				    $cuerpo .= "<br>\r\n";
				    $cuerpo .= "<br>\r\n";	
				    ...
				    $cuerpo .= "<br>\r\n";
				    $cuerpo .= "<br>\r\n";	
				    $cuerpo .= "<div><font size='3'>Dirección de Recursos Humanos</font></div>\r\n";
		...
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" )
			...
			if(!$this->input->is_ajax_request()) {
				...
				$registro=array(
					...
					'fecha_inicio'=>$this->input->post('fechainicio'),
					'fecha_terminacion'=>$this->input->post('fechafinal'),					
					'estado'=>$this->$this->input->post('estado')
				);
...
// ************* <<===== Excel de Consulta Documentos pendientes Contrato =====>*************//
	...
	public function listar_anexos() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
            	...
            	$query = $this->general_model->consulta_personalizada($campos, '(SELECT ld.id_listado AS "Id", ld.nombre AS "Nombre", ct.id_ingreso AS "id_ingreso" FROM ckeklist_contratosp AS cc INNER JOIN listado_documentos AS ld ON find_in_set(ld.id_listado, cc.listado_documentos) INNER JOIN ingreso_personal ct ON ct.id_cargo=cc.id_cargo) AS t1 LEFT JOIN ingreso_personal_anexos ca ON t1.id=ca.id_checklist_contratos  AND t1.id_ingreso =ca.id_ingresop', ' t1.id_ingreso = "'.$idreg.'" ', '', 0, 0);
	...
	public function listar_documentos() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
    			...
    			$query = $this->general_model->consulta_personalizada($campos, 'ckeklist_contratosp AS cc LEFT JOIN listado_documentos AS ld ON find_in_set(ld.id_listado, cc.listado_documentos)', 'cc.id_cargo='.$idreg.' AND ld.estado = "1"', 'ld.id_listado', 0, 0);
	...
	public function cargar_centros() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			$query = $this->general_model->consulta_personalizada($campos,'centroscostos', 'id_linea_costos ='.$idlinea.'', '', 0, 0);
