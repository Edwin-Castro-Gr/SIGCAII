...
class A_terceros extends CI_Controller {
	...
	public function pdf() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" )
	        ...
	        $pdf->hoja = 'LETTER';
	        $pdf->SetTitle("SIGCA - Listado de Terceros", true);
	        $pdf->SetLeftMargin(7);
            ...
            $campos =' t.tipo_tercero AS "Tipo", t.numero_id AS "Numero", t.razon_social AS "Razon", t.telefono_contacto AS "Telefono", t.correo_contacto AS "Correo", CASE WHEN t.proveedor_critico ="0" THEN "Si" ELSE "No" END AS "Critico" , CASE WHEN cc.estado="1" THEN "Activo" ELSE "Inactivo" END AS "Estado" ';
			...
			$pdf->Cell(16,5,utf8_decode("TIPO TERCERO"),'LTRB',0,'C',$fill);
			$pdf->Cell(35,5,utf8_decode("NUMERO"),'LTRB',0,'C',$fill);
			$pdf->Cell(80,5,utf8_decode("RAZON SOCIAL / NOMBRE"),'LTRB',0,'C',$fill);
			$pdf->Cell(25,5,utf8_decode("TELEFONO"),'LTRB',0,'C',$fill);
			$pdf->Cell(45,5,utf8_decode("EMAIL"),'LTRB',0,'C',$fill);
			$pdf->Cell(25,5,utf8_decode("PROVEDOR CRITICO"),'LTRB',0,'C',$fill);
	        ...
	        foreach ($encabezados as $row) {
	        	$pdf->Cell(4,5,' ',0,0,'C',false);
                $pdf->Cell(10,5,($row->Tipo),'LTRB',0,'C',$fill);
                $pdf->Cell(35,5,utf8_decode($row->Numero),'LTRB',0,'C',$fill);
                $pdf->Cell(80,5,utf8_decode($row->Razon),'LTRB',0,'C',$fill);
                $pdf->Cell(25,5,utf8_decode($row->Telefono),'LTRB',0,'C',$fill);
                $pdf->Cell(45,5,utf8_decode($row->Correo),'LTRB',0,'C',$fill);
                if($row->Critico == "Si")
                	$pdf->Cell(20,5,$row->Critico,'LTRB',0,'C',$fill);
                else
                	$pdf->Cell(20,5,$row->Critico,'LTRB',0,'C',!$fill);
