...
class R_actas extends CI_Controller {
	...
	public function revisar($id) {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['titulo']="Revisar de Actas";
			$data_usua['origen']="Registros";
			$data_usua['contenido']='actas/revisar';
			$data_usua['entrada_js']='_js/r_actas.js';
			$data_usua['librerias_css']='<!-- DataTables -->
			<link rel="stylesheet" type="text/css"  href="'.base_url('plugins/datatables.net-bs4@1.10.24/css/dataTables.bootstrap4.min.css').'">			
			<link rel="stylesheet" type="text/css" href="'.base_url('plugins/datatables.net-buttons-bs4@1.7.0/css/buttons.bootstrap4.min.css').'">
	...
	public function aprobar($id) {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			$data_usua['origen']="Registros";
			$data_usua['contenido']='actas/aprobar';
			$data_usua['entrada_js']='_js/r_actas.js';
			$data_usua['librerias_css']='<!-- DataTables -->
			<link rel="stylesheet" type="text/css"  href="'.base_url('plugins/datatables.net-bs4@1.10.24/css/dataTables.bootstrap4.min.css').'">			
			<link rel="stylesheet" type="text/css" href="'.base_url('plugins/datatables.net-buttons-bs4@1.7.0/css/buttons.bootstrap4.min.css').'">
	...
	public function listar_tabla() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) {
			...
			if(!$this->input->is_ajax_request()) {
				...
				echo listar_actas_tabla('WEB',$usuario);
			}
		}
	}
		...
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="") 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$registro=array(
					...
					'fecha_reunion'=> date("Y-m-d", strtotime($this->input->post('fechaR'))), 
					...
					'temas_tratados'=>$this->input->post('temas'),
					'detalle_temas'=>$this->input->post('temasD'),
					'decisiones'=>$this->input->post('decisiones'),
				...
				if($query >= 1) {
					...
					while ($i <= $numTarea){
						...
						$registro=array(
							...
							'id_acta '=>$query, 
							'responsable'=>$this->input->post('participanteT'.$i.''), 
				...
				if($query1 >= 1) {
					...
					$dir = 'archivo_actas';
			 		...
			 		if (!file_exists('archivos/'.$this->session->userdata('C_basedatos'))) {
				 		...
				 		if (!file_exists('archivos/'.$this->session->userdata('C_basedatos').'/archivos/')) {
					 		mkdir('archivos/'.$this->session->userdata('C_basedatos').'/archivos/', 0777, true);
					 		if (!file_exists('archivos/'.$this->session->userdata('C_basedatos').'/archivos/'.$dir.'/')) {
						 		mkdir('archivos/'.$this->session->userdata('C_basedatos').'/archivos/'.$dir.'/', 0777, true);
						 	}
					 	}
				 	}elseif (!file_exists('archivos/'.$this->session->userdata('C_basedatos').'/archivos/')) {
					 	mkdir('archivos/'.$this->session->userdata('C_basedatos').'/archivos/', 0777, true);
				 	}elseif (!file_exists('archivos/'.$this->session->userdata('C_basedatos').'/archivos/'.$dir.'/')) {
				 		mkdir('archivos/'.$this->session->userdata('C_basedatos').'/archivos/'.$dir.'/', 0777, true);
				 	}

					$ruta = './archivos/'.$this->session->userdata('C_basedatos').'/archivos/'.$dir.'/';  
					$rutag ='archivos/'.$this->session->userdata('C_basedatos').'/archivos/'.$dir.'/';
					...
					while ($ia <= $numAsistentes){
						...
						$fileB64 = $this->input->post('file64F'.$ia.'');
						$fileB64 = str_replace('data:image/png;base64,', '', $fileB64);
						$firma_base64 = base64_encode($fileB64);

						if ($this->upload->do_upload('fileF'.$ia.'')){
	...
	public function actualizar() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				if($this->input->post('doc_relacionado')!=""){
					$docrela = $this->input->post('doc_relacionado');
					$val_docrela = implode(',', (array) $docrela);
				}else{
					$val_docrela = '';
				}		
				...
				//ESTABLECER LA RUTA DONDE SE GUARDA EL ARCHIVO
					...
					$registro=array(
						...
						'documento_relacionado'=>$val_docrela,
						'origen_formato'=>$this->input->post('origen_formato'),
				...
				if($query=="OK"){
					//ENVIAR NOTIFICACION A USUARIO CALIDAD
					$id_solicitud = $this->input->post('idreg');	
					if($this->session->userdata('C_id_usuario')==$this->input->post('idusuarioregsol')){
						...
						$query = $this->general_model->insert('notificaciones', $registro2);
						if($query >= 1) 
							//ENVIAR TAREA A USUARIO CALIDAD
						...
						switch($estado) {
							case '0': //PENDIENTE
								$id_usuario_2 = $this->input->post('idusuarioregsol');
								$observacion ="Corregir solicitud N° '".$id_solicitud."'";
							...
							case '2': //RECHAZADA
								$id_usuario_2 = $this->input->post('idusuarioregsol');
								$descripcion ="Solicitud N°".$this->input->post('idreg')." Fue rechazada";
	...
	public function cargar_docrelacionado() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				if($id_docrel != NUll){
					$query = $this->general_model->consulta_personalizada($campos, 'documentos d','d.id_documento IN ('.				$docrela.')', 'd.nombre', 0, 0);
				;
				}else if($id_subpro != ""){
