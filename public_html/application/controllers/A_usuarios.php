	...
	public function cargar_registro() {
		if(!defined('CON_id_usuario') && $this->session->userdata('C_id_usuario')=="" ) 
			...
			if(!$this->input->is_ajax_request()) {
				...
				$campos = ' id_usuario, usuario, AES_DECRYPT(clave,"-Qsc.725943!") AS clave, nombre, apellido, telefono, email, id_empleado, perfil, estado';
				$query = $this->general_model->consulta_personalizada($campos, 'usuarios', ' id_usuario = "'.$id.'" ', '', 0, 0);
