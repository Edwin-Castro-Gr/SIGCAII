$(function () {
    ...
    if ($('#opc_pag').val() == "listado") {
        ...
        function cargar_listado() {
            ...
            $.post("/c_ingresop/listar_tabla", {}, function (data_carg) {
                //alert(data_carg);
        ...
        $('#empleados_ingresop').select2();
        $('#empleados_jefeinm').select2();
        $('#cargos_ingresop').select2({
            placeholder:'Seleccione un Cargo..',
            allowClear: true
        });
        $('#cargos_ingresop').val("").trigger('change')
        ...
        $.post("/c_ingresop/listar_anexos",{id_ingreso: ""+$('#idregistro').val()+"", tipo_cont: ""+tipo_cont+"" }, function(data_carg){
            //alert(data_carg);
            $("#accordionA").empty();
    ...
    $(document).on("click", function (event) {
        ...
        if (datos[0] == "btneditar") {
            idreg = datos[1];
            window.open('/c_ingresop/modificar/'+idreg,'_parent');
        }
        ...
        if(dato =="btn_guardar_empleado"){
          ...
          if(ban==1) {  
            ...
            $.post("/c_ingresop/guardar_empleado", datos_form, function(data_form){
              Swal.close();
                if(data_form=="1") {
                //jQuery(function(){
                    Swal.fire({
                    ...
                    .then((willDelete) => {
                        ...
                        $.post("/c_ingresop/cargar_empleado",{emple: ""+cedula+""}, function(data_terce){
        ...
        if (dato == "btn_pdf") {
            window.open('/c_ingresop/pdf', '_blank');
        }

        if (dato == "btn_consulta") {
            window.open('/c_ingresop/consultas', '_parent');
        }

        if (dato == "btn_consulta_todos") {
            window.open('/c_ingresop/consulta_cont_docpend', '_blank');
        }


        if (dato == "btn_excel") {
            window.open('/c_ingresop/excel', '_blank');
        }
    });
    ...
    $(document).on("change", function(event){
      ...
      var dato = event.target.id;
        ...
        if(dato == "cargos_ingresop") {
            ...
            $.post("/c_ingresop/listar_documentos",{cargos_ingresop: ""+cargo+"", tipocontrato: ""+tipo_contrato+"",}, function(data_carg){
              //alert(data_carg);
              $("#accordionA").empty();
              $("#accordionA").html(data_carg);              
            });
    ...
    $('#cedula_empleado').keyup(function(e) {
        ...
        if(e.keyCode == 13) {
          ...
          $.post("/c_ingresop/cargar_empleado",{emple: ""+cedula+""}, function(data_emple){
    ...
    $('#cedula').blur(function(e){
        e.preventDefault();
        $.post("/c_ingresop/consultar_empleado", {cedula:($('#cedula').val())}, function(data_preg){
            if(data_preg != false){
    ...
    function guardar_registro() { 
        ...
        $.ajax({
            ...
            if(res >= 1) {
              ...
              .then((willDelete) => {
                window.open('/c_ingresop/index','_parent');            
              }); 
            } else {
