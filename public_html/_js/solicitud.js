$(function () {
    ...
    if ($('#opc_pag').val() == "ingreso") {
        ...
        if (docrelacionado !== "" ){
            document.getElementById("ck_d_relacionado").checked = true;
            $('#lbldoc_relacionado').css("display", "block");
            $('#divdoc_relacionado').css("display", "block");  
            
            $.post("/d_solicitud/cargar_docrelacionado",{idsubpr: ""+id_subproceso+"", iddocrel: ""+docrelacionado+""}, function(data_form){
        
                $('#doc_relacionado').html(data_form);
            });
        }else{
            document.getElementById("ck_d_relacionado").checked = false;
            $('#lbldoc_relacionado').css("display", "none");
            $('#divdoc_relacionado').css("display", "none");             
        }
        $('#empleados_solicitud').select2();
        ...
        // $('#empleadosMR_revisa').val(explode('#idusuariorevisa');
    ...
    $(document).on("change", function(event){
        ...
        if(dato == "ck_d_relacionado") {
            ...
            let docrelacionado = $('#iddocrelacionado').val();
            
            if(ck==true){
                // alert(id_macro+id_proceso+id_subproceso+docrelacionado);
                $.post("/d_solicitud/cargar_docrelacionado",{idmacro: ""+id_macro+"",idproc: ""+id_proceso+"",idsubpr: ""+id_subproceso+"", iddocrel: ""+docrelacionado+""}, function(data_form){
                    $('#lbldoc_relacionado').css("display", "block");
                    $('#divdoc_relacionado').css("display", "block");
                    $('#doc_relacionado').html(data_form);
                });                
        ...
        if(dato == "subproceso") {
            ...
            if(document.getElementById("ck_d_relacionado").checked == true){
                $.post("/d_solicitud/cargar_docrelacionado",{idmacro: ""+id_macro+"",idproc: ""+id_proceso+"",idsubpr: ""+id_subproceso+"", iddocrel: ""+docrelacionado+""}, function(data_form){
                    $('#lbldoc_relacionado').css("display", "block");
                    $('#divdoc_relacionado').css("display", "block");
                    $('#doc_relacionado').html(data_form);
