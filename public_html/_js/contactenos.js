$(function () {
  ...
  if ($("#opc_pag").val() == "reportes") {
    var vswitch = 'false';
    cargar_listado(vswitch);

    function cargar_listado(vswitch) {
      Swal.fire({
      ...
      $.post("/contactenos/listar_tabla",{vswitch:""+vswitch+""}, function (data_carg) {
        //alert(data_carg);
    ...
    alert($("#estado").val());
    if($("#estado").val() == 0){
      $('#btn_guardar_gestion').prop('disabled', 'disabled');
    }else{
      $('#btn_guardar_gestion').prop('disabled', false);
    }
  ...
  $(document).on("click", function (event) {
    ...
    if (dato == "btn_sucess_modal") {
      ...
      // alert(fecha[0]);
      // alert(fecha[1]);
      window.open("/contactenos/excel/" + idmes, "_blank");
    }
